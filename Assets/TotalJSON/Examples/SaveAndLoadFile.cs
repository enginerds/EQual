...
namespace Leguar.TotalJSON.Examples {
	...
	public class SaveAndLoadFile : MonoBehaviour {
		...
		private void saveJsonObjectToTextFile(JSON jsonObject) {
			string jsonAsString = jsonObject.CreatePrettyString(); // Could also use "CreatePrettyString()" to make more human readable result, it is still valid JSON to read and parse by computer
